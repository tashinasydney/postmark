...
- **The lake caves** show a second portal (blue-and-white spiral, same trick as Pandara's, titled "Upcoming Event") and the Tributes below it, with a bit more air between the two now (`.stagepage-single` carries its own `gap`) — Jetto's closeout card, Limen's surviving note (in a protective case), and an open invitation for the Illuminator to add her own housewarming gift. None are painted yet, so they're red placeholder squares until she has hands free. Update these to real images as each one lands.
